...
var_list5 <- seq(1,1,length.out = ite)
...
for(iteite in 1:ite){
  ...
  cf2 <- function(gamma.){
    ...
    ccc9 <- delta*exp(gamma.*y_transform)*(y_transform)
    ccc10 <-  delta*exp(gamma.*y_transform)
    g_list9 <- c(1,1,2*x_cate)
    for(i in 0:x_cate-1){
      for(j in 0:1){
        g_list9[1+i+x_cate*j]<- sum(ccc9[comb_x_z_transform==(i+x_cate*j)])/sum(ccc10[comb_x_z_transform==(i+x_cate*j)])
      }
    }
    g_list10 <- apply(apply(comb_x_z, 2, function(x) x*g_list9),2,sum)
    
    ##aaa <- (delta/pseudo_pi-1.0)*(g_list6-apply(apply(x, 2, function(x) x*g_list3),2,sum))
    aaa <- (1-pseudo_pi)/pseudo_pi*(g_list6-g_list4)**2
    bbb <- (1-pseudo_pi)/pseudo_pi*delta*(y_transform-g_list6)*(g_list6-g_list4)
  ...
  cf4_2 <- function(gamma.){
    ...
    ccc6 <- delta*exp(gamma.*y_transform)
    for(kkkk in 0:x_cate-1){
      g_list4[kkkk+1] <- sum(ccc5[x_transform==kkkk])/sum(ccc6[x_transform==kkkk])
    }
    g_list7 <- apply(apply(comb_x_z, 2, function(x) x*g_list4),2,sum)
    
    g_list5 <- c(1,1,2*x_cate)
    ccc7 <- delta*exp(gamma.*y_transform)*(y_transform)
    ccc8 <- delta*exp(gamma.*y_transform)
    for(i in 0:x_cate-1){
      for(j in 0:1){
        g_list5[1+i+x_cate*j]<- sum(ccc7[comb_x_z_transform==(i+x_cate*j)])/sum(ccc8[comb_x_z_transform==(i+x_cate*j)])
      }
    }
    ...
    bbb <- (1-pseudo_pi)/pseudo_pi*delta*(y_transform-g_list7)*(z_transform-apply(apply(x, 2, function(x) x*g_list3),2,sum))
  ...
  cf5_2 <- function(gamma.){
    ...
    g_list4 <- apply(apply(x, 2, function(x) x*g_list3),2,sum)
    
    ccc7 <- delta*exp(gamma.*y_transform)*(y_transform)
    ccc8 <- delta*exp(gamma.*y_transform)/pseudo_pi
    g_list7 <- c(1,1,2*x_cate)
    for(i in 0:x_cate-1){
      for(j in 0:1){
        g_list7[1+i+x_cate*j]<- sum(ccc7[comb_x_z_transform==(i+x_cate*j)])/sum(ccc8[comb_x_z_transform==(i+x_cate*j)])
      }
    }
    g_list8 <- apply(apply(comb_x_z, 2, function(x) x*g_list7),2,sum)
    
    ccc9 <- delta*exp(gamma.*y_transform)*g_list8
    ccc10 <- delta*exp(gamma.*y_transform)
    g_list9<- c(1,1,x_cate)
    for(kkkk in 0:x_cate-1){
      g_list9[kkkk+1] <- sum(ccc9[x_transform==kkkk])/sum(ccc10[x_transform==kkkk])
    }
    g_list10 <- apply(apply(x, 2, function(x) x*g_list9),2,sum)
    ##aaa <- (delta/pseudo_pi-1.0)*(g_list6-apply(apply(x, 2, function(x) x*g_list3),2,sum))
    aaa <- (1-pseudo_pi)/pseudo_pi*(g_list8-g_list10)**2
    bbb <- (1-pseudo_pi)/pseudo_pi*delta*(y_transform-g_list4)*(g_list8-g_list10)
    return(mean(aaa)/(n*mean(bbb)*mean(bbb)))
    ###return(var(aaa))
  }
  print("estimation")
  print(cf5_2(true_gamma))
  var_list5[iteite] <- cf5_2(true_gamma)
  
  ### Calculate CI
  if(-1.96*sqrt(cf5_2(boku5$minimum))+boku5$minimum<true_gamma){
    if(1.96*sqrt(cf5_2(boku5$minimum))+boku5$minimum>true_gamma){
      count_list5 <-count_list5+ 1.0 
    }
  }
}
