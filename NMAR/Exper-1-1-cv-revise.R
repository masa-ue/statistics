...
for(iteite in 1:ite){
  ...
  cf2 <- function(gamma.){
    ...
    bbb <- (1-pseudo_pi)*(y_transform-g_list8)*(g_list6-g_list4)
  ...
  cf4_2 <- function(gamma.){
    ...
    bbb <- (1-pseudo_pi)*(y_transform-apply(apply(x, 2, function(x) x*g_list4),2,sum))*(z_transform-apply(apply(x, 2, function(x) x*g_list3),2,sum))
