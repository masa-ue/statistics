...
for(iteite in 1:ite){
  ...
  cf2 <- function(gamma.){
    ...
    bbb <- (1-pseudo_pi)*(g_list10-g_list8)*(g_list6-g_list4)
  ...
  ### Calculate CI
  if(-1.96*sqrt(cf2(true_gamma))+boku2$minimum<true_gamma){
    if(1.96*sqrt(cf2(true_gamma))+boku2$minimum>true_gamma){
  ...
  cf4_2 <- function(gamma.){
    ...
    bbb <- (1-delta)*(g_list7-g_list6)*(z_transform-apply(apply(x, 2, function(x) x*g_list3),2,sum))
  ...
  ### Calculate CI
  if(-1.96*sqrt(cf4_2(true_gamma))+boku4$minimum<true_gamma){
    if(1.96*sqrt(cf4_2(true_gamma))+boku4$minimum>true_gamma){
  ...
  cf5_2 <- function(gamma.){
    ...
    bbb <- (1-pseudo_pi)*(delta*y_transform/pseudo_pi-g_list4)*(g_list8-g_list10)
    return(mean(aaa)/(n*mean(bbb)*mean(bbb)))
    ###return(var(aaa))
  }
  print("estimation")
