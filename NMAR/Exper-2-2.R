...
for(iteite in 1:ite){
  ...
  print("EM Score")
  gamma_estimate2_list =seq(0,0,length.out = 500)
  gamma_known <- 0.5
  mmmm <- 500
  ...
  print("EM P-EE-1")
  mmmm <- 500
  gamma_estimate2_list =seq(0,0,length.out = mmmm)
  gamma_known <- 0.5
  large_matrix <- matrix(1,n,mmmm)
  for(iii in 1:n){
    large_matrix[iii,] <- rnorm(mmmm, mu[iii], sigma_)
...
###print(mean(gamma_estimate-true_gamma))
