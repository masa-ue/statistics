...
y_list3 <- seq(1,1,length.out = ite) 
...
for(iteite in 1:ite){
  ...
  sigmoid <- function(xxx,yyy){
    return(1.0/(1.0+exp(-1.3-0.3*(xxx**0.5)-0.2*xxx+0.6*yyy)))
  }
  ...
  gamma <- 0.6
  ccc10 <- npksum(txdat = x1, tydat = delta*exp(gamma*y),ckerorder=2,bws=0.3)
  ccc11 <-  npksum(txdat = x1, tydat = 1-delta,ckerorder=2,bws=0.3)
  ...
  likelihood <- function(gamma.,y.=y){
    pseudo_pi <- 1.0/(1.0+exp(gamma.[1]+gamma.[2]*x1+gamma.[3]*(x1**0.5)+gamma.[4]*y.))
  ...
  for(tttt in 1:1){
    likelihood2 <- function(gamma.){
      ...
      g_list4 <- seq(1,1, length.out = nn)
      for(iii in 1:nn){
        sum3 <- seq(1,1,length.out = mmmm)
        sum4 <- seq(1,1,length.out = mmmm)
        for(jjj in 1:mmmm){
          sum3[jjj] <- exp(gamma.*large_matrix[iii,jjj])*large_matrix[iii,jjj]*as.vector(ccc1$ksum[iii]/(ccc1$ksum[iii]+ccc2$ksum[iii]*exp(gamma.*large_matrix[iii,jjj])))
  ...
  gamma_estimate3[iteite] <- gamma_known2
  print(gamma_estimate3[iteite])
  
  ### DB
  
  ccc1 <- npksum(txdat = x1, tydat = delta*exp(gamma_estimate3[iteite]*y),bws = 0.25)
  ccc2 <- npksum(txdat = x1, tydat = 1-delta,bws = 0.25)
  pseudo_pi <- as.vector(ccc1$ksum/(ccc1$ksum+ccc2$ksum*exp(gamma_estimate3[iteite]*y)))
  y_estimate3_db <- mean(na.omit((y-mu)*delta/pseudo_pi+mu))
  print(y_estimate3_db)
  ...
  ccc1 <- npksum(txdat = x1, tydat = delta*exp(boku4$minimum*y),bws = 0.25)
  ccc2 <- npksum(txdat = x1, tydat = 1-delta,bws = 0.25)
  g_list2 <- as.vector(ccc1$ksum/ccc2$ksum)
  ...
  likelihood5 <- function(gamma.){
    ccc1 <- npksum(txdat = x1, tydat = delta*exp(gamma.*y),ckerorder=2,bws=0.3)
    ccc2 <-  npksum(txdat = x1, tydat = 1-delta,ckerorder=2,bws=0.3)
    pseudo_pi <- as.vector(ccc1$ksum/(ccc1$ksum+ccc2$ksum*exp(gamma.*y)))
    aaa <- (delta/pseudo_pi-1.0)*x2
    return(sum(aaa)*sum(aaa))
  }
  ...
  y_list3[iteite] <- y_estimate3_db
}
print(sqrt(var(y_list)))*1.96
print(sqrt(var(y_list2)))*1.96
print(sqrt(var(y_list3)))*1.96
print(mean((y_list)))
print(mean((y_list2)))
print(mean((y_list3)))
