...
for(iteite in 1:ite){
  ...
  cf2_2 <- function(gamma.){
    ...
    ccc1 <- (1-pseudo_pi)/pseudo_pi*delta*(y_transform-g_list4)*(y_transform-g_list4)
    ccc2 <- (1-pseudo_pi)/pseudo_pi*delta*(y_transform-g_list6)*(y_transform-g_list6)
    return(c(mean(ccc1),mean(ccc2)))
    ###return(var(aaa))
  }
  print("coverage")
  ###print(cf2_3(boku2$minimum)*1000/n)
  auxi <- cf2(true_gamma)
  auxi2 <- cf2_2(true_gamma)[1]
  auxi3 <- cf2_2(true_gamma)[2]
  ###print(var(auxi))
  print(auxi2)
  print(auxi3)
  print(var(g_list4[delta==0]+(delta/pseudo_pi)*(y_transform-g_list4[delta==0])-auxi2*auxi)*1000/n)
  print("coverage2")
  var_list2_out[iteite] <-  var(g_list4[delta==0]+(delta/pseudo_pi)*(y_transform-g_list4[delta==0]))*1000/n
